"""
Add Nexus to Windows startup so it runs when the computer turns on.

Supports two modes:
  --exe    Uses the built executable (dist/Nexus/Nexus.exe)
...
def install(use_exe=False):
    ...
    bat_path = os.path.join(startup_folder, "Nexus.bat")

    if use_exe:
        exe_dir = os.path.join(here, "dist", "Nexus")
        exe_path = os.path.join(exe_dir, "Nexus.exe")
...
"""
    ...
    print(f"Nexus will now start automatically when you log in.")
    print()
...
def uninstall():
    ...
    if os.path.exists(bat_path):
        ...
        print("Nexus will no longer start automatically.")
    else:
        print("Nexus is not in startup.")
